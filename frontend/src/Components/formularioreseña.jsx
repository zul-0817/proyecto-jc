...
function FormularioReseña({ reseñaEditar, onClose, onSuccess }) {
  ...
  useEffect(() => {
    if (reseñaEditar) {
      setFormData({
        ...
        textoResena: reseñaEditar.textoResena || '',
        horasJugadas: reseñaEditar.horasJugadas || 0,
        dificultad: reseñaEditar.dificultad || 'Media',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!formData.textoResena.trim() || formData.textoResena.length < 10) {
      ...
      if (reseñaEditar) {
        ...
        mostrarNotificacion('Reseña actualizada correctamente', 'exito');
      } else {
        await crearResena(dataToSend);
        mostrarNotificacion('Reseña creada correctamente', 'exito');
      }
      ...
      if (err.response && err.response.data) {
        setError(err.response.data.mensaje || err.response.data.error || err.message);
      } else {
          ...
          <form className="formulario-resena" onSubmit={handleSubmit}>
            ...
            <div className="form-group">
              ...
              <div className="contador-caracteres">
                {formData.textoResena.length} / 2000 caracteres
              </div>
            </div>
